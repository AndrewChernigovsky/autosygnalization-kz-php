...
class Share
{
    public function getShare()
    {
      ...
      ob_start();
        ?>
        <div class="share">
            <p style="background-image: url(<?= htmlspecialchars($path . '/assets/images/vectors/share-icon.svg'); ?>);">Поделиться:</p>
                ...
                <?php foreach ($icons as $icon): ?>
                  <li>
                    <a href="<?= htmlspecialchars($icon['href']); ?>" title="<?= htmlspecialchars($icon['name']); ?>">
                      <img src="<?= htmlspecialchars($icon['path']); ?>" width="<?= htmlspecialchars($icon['width']); ?>" height="<?= htmlspecialchars($icon['height']); ?>" alt="<?= htmlspecialchars($icon['name']); ?>" />
