  ...
  try {
    ...
    if (success && data) {
      contacts.value = data.filter(
        (item: ContactItem) => item.type === 'social'
      );
    } else {
...
const updateContact = async (id: number): Promise<void> => {
  try {
    ...
    formData.append('content', item.content ? item.content : item.link || '');
    formData.append('link', item.link ? item.link : item.content || '');
...
const createContact = async (): Promise<void> => {
  try {
    ...
    formData.append(
      'link',
      newContact.value.link ? newContact.value.link : newContact.value.content
    );
    formData.append('type', 'social');
...
<template>
  <div class="container">
    <h2 class="contacts-title">Социальные сети</h2>

    <div class="add-contact-wrapper">
      <h2 class="add-contact-title">Добавить новую социальную сеть</h2>
      <form @submit.prevent="createContact" class="add-contact-form">
        ...
        <div class="input-group">
          <label for="link-create">Ссылка:</label>
          <input type="text" id="link-create" v-model="newContact.link" />
        </div>
    ...
    <div>
      <div class="contact-list">
        ...
        >
          ...
          <div class="input-group">
            <label :for="'link-' + item.contact_id">Ссылка:</label>
            <input
              type="text"
