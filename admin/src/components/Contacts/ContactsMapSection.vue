...
const getContacts = async (): Promise<void> => {
  try {
    ...
    if (success && data) {
      contacts.value = data.filter((item: ContactItem) => item.type === 'map');
    } else {
...
<template>
  <div class="container">
    <h2 class="contacts-title">Карта</h2>

    <div class="add-contact-wrapper">
      <h2 class="add-contact-title">Добавить новую карту</h2>
      <div class="add-contact-form">
        <div class="input-group">
          <label :for="'link-create'">Ссылка на карту:</label>
        ...
        <div v-for="item in contacts" :key="item.contact_id" class="nav-item">
          <div class="input-group">
            <label :for="'content-' + item.content?.replace(/[+\s]/g, '')"
              >Ссылка на карту:</label
            >
            <input
              type="text"
              :id="'link-' + item.link?.replace(/[+\s]/g, '')"
              v-model="item.link"
            />
          </div>
