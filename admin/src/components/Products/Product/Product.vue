...
function markDirty() {
  ...
  console.log('🔍 [MARK_DIRTY] isSaving:', isSaving.value);
  isDirty.value = true;
}

function resetDirty() {
  console.log('🔍 [RESET_DIRTY] Вызывается resetDirty');
  console.log('🔍 [RESET_DIRTY] isSaving:', isSaving.value);
  isDirty.value = false;
}
...
watch(
  () => isDirty.value,
  (val) => {
    console.log('🔍 [WATCHER] isDirty изменился:', val);
...
watch(
  ...
  (newVal) => {
    ...
    if (editingProduct.value && !isSaving.value) {
      ...
      console.log(
        '🔍 [WATCHER] НЕ обновляем - isSaving или editingProduct отсутствует'
      );
    }
  },
  { deep: true }
...
watch(
  ...
  (newVal) => {
    ...
    try {
      ...
      const isChanged = a !== b;
      console.log('🔍 [WATCHER] Сравниваем объекты, isChanged:', isChanged);
...
const onUpdateTabIconPath = (
  ...
  console.log(
    '🔍 [UPDATE] onUpdateTabIconPath вызвана:',
    productId,
    tabIndex,
    itemIndex,
    newPath
  );
  console.log('🔍 [UPDATE] editingProduct.value:', editingProduct.value);
  if (!editingProduct.value || editingProduct.value.id !== productId) {
    console.log(
      '🔍 [UPDATE] НЕ обновляем - неверный productId или editingProduct отсутствует'
    );
    return;
  }

  // Обновляем путь в editingProduct
  if (
    ...
    console.log(
      '🔍 [UPDATE] Обновляем путь в editingProduct:',
      tabIndex,
      itemIndex,
      newPath
    );
    editingProduct.value.tabs[tabIndex].content[itemIndex]['path-icon'] =
      newPath;
    console.log('🔍 [UPDATE] Путь обновлен в editingProduct');
  } else {
    console.log('🔍 [UPDATE] НЕ удалось обновить - структура tabs не найдена');
  }
};
